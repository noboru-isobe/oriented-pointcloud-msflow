...
Usage:
    uv run python scripts/experiments/render_local_merger_movie.py
Output:
    results/reports/local_merger_768_movie.mp4
"""
...
SPLICE_STEP = 2014
EVENTS = {
    1433: ("t_act (WB - CC activation)", "0.55"),
    1440: ("t_grid (support merger)", "0.7"),
    1825: ("t_quot (QUOTIENT: bulks 2 - 1)", "tab:red"),
    2014: ("t_splice (SPLICE: loops 2 - 1, N 376 - 304)", "tab:purple"),
}
...
class LocalMergerAnimator(VarifoldAnimator):
    ...
    def setup_figure(self):
        ...
        self.axes[0].set_title(
            "MS flow: two-ellipses LOCAL merger (H = 768, t = 0 run)")
...
def main():
    ...
    setup_animation_backend()
    # ---- stitched per-step series -------------------------------
    la3 = json.load(open(R / "two_ellipses_la3_768.json"))["series"]
    l1q = json.load(open(R / "two_ellipses_l1q_768.json"))["series"]
    s2 = json.load(open(R / "two_ellipses_s2long2_768.json"))["series"]
    per, vol = {}, {}
    for r in la3 + l1q:
        per[r["step"]] = r["P_frozen"]
        vol[r["step"]] = r["A"][0] + r["A"][1]
    for r in s2:
        per[SPLICE_STEP + 1 + r["step"]] = r["P_frozen"]
        vol[SPLICE_STEP + 1 + r["step"]] = r["A"]
    last = max(per)
    ...
    for tag, off, lab in (("_la3_768", 0, ""), ("_l1q_768", 0, "")):
        ck = torch.load(R / f"two_ellipses{tag}_states.pt",
                        weights_only=True)
        for k in sorted(x for x in ck if isinstance(x, int)):
            frames.append((k + off, OrientedPointCloudVarifold(
                positions=ck[k]["positions"],
                angles=ck[k]["angles"]), lab))
    blob = torch.load(R / "two_ellipses_s1b_768_spliced_state.pt",
                      weights_only=False)
    frames.append((SPLICE_STEP, OrientedPointCloudVarifold(
        positions=blob["positions"], angles=blob["angles"]),
        "spliced (zero-time)"))
    ck = torch.load(R / "two_ellipses_s2long2_768_states.pt",
                    weights_only=True)
    for k in sorted(x for x in ck if isinstance(x, int)):
        frames.append((SPLICE_STEP + 1 + k, OrientedPointCloudVarifold(
    ...
    anim = LocalMergerAnimator(varifolds, steps, labels, P_series,
                               V_series, cfg)
    out = Path("results/reports/local_merger_768_movie.mp4")
    anim.setup_figure()
