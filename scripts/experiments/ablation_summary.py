...
def _draw_normal_arrows(ax, pos, angles, xlim, ylim,
                        num_arrows: int = 24) -> None:
    """Overlay subsampled outward-normal arrows on a boundary scatter so
    each cell's orientation field is visible (mirrors what the showcase
    mp4s draw via VarifoldAnimator._update_normal_arrows). Subsamples to
    keep the figure readable at small panel sizes."""
    n = len(pos)
    if n == 0:
        return
    stride = max(1, n // num_arrows)
    sub = slice(None, None, stride)
    nx = np.cos(angles[sub]); ny = np.sin(angles[sub])
    arrow_len = 0.08 * (xlim[1] - xlim[0])
    ax.quiver(pos[sub, 0], pos[sub, 1], nx, ny,
              angles="xy", scale_units="xy", scale=1.0 / arrow_len,
              width=0.006, headwidth=4, headlength=5,
              color="k", alpha=0.6, zorder=3)
...
def plot_cell_evolution(cell: Cell, data: dict, npz, out_path: Path,
    ...
    for j, step in enumerate(SNAPSHOT_STEPS):
        ...
        _draw_normal_arrows(ax, pos, ang, xlim, ylim)
